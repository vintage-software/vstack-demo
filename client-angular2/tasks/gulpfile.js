...
gulp.task('clean', 'Clean the solution by removing any compiled files', (done) => {
    del(CONFIGS.map(config => config.buildLocations.clean), {force: true}, done);
});
...
gulp.task('_update.version', 'Create version for CSS/JS references', () => {
    let tasks = CONFIGS.map(config => {
        ...
        return target
                .pipe(inject(sources, { transform: (filepath) => injectVersion(filepath, config.buildLocations.rootDist), addRootSlash: false }))
...
gulp.task('_update.template-version', 'Create version for HTML template references', () => {
    let tasks = CONFIGS.map(config => {
        return gulp.src(config.html.templateUrlReferences)
                ...
                .pipe(replace(new RegExp(`templateUrl: '${config.app.baseName}/`), `templateUrl: '${removeRootDistPath(config.buildLocations.html, config.buildLocations.rootDist)}`))
...
function build(done) {
    runSequence(
        'clean',
        ['_build.typescript', '_build.images', '_build.fonts', '_build.html', '_build.javascript', '_build.sass', '_sass-lint'], // , '_ts-lint'
        '_update.template-version',
		'_update.version', done);
}

function injectVersion(filepath, rootRegEx) {
    // remove web root path and then add version 
    arguments[0] = removeRootDistPath(filepath, rootRegEx) + '?v=' + getVersion();
    return inject.transform.apply(inject.transform, arguments);
}

function removeRootDistPath(filepath, rootRegEx) {
    return filepath.replace(rootRegEx, '');
}
